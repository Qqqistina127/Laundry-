...
{
	char answer;
	string nameServ[10],servDetail[10],servPrice[10],name,details;
	double price;
...
void washerServ(string& name,string& details,double& price)
{
	...
	if (ansService=='y')
	{
		...
		temp=toupper(temp);
		//ada logic error
		while (code!='a'||code!='b'||code!='c'||temp!='C'||temp!='W'||temp!='H')
		{
			cout<<"You've entered invalid code or invalid temperature. Please enter again.";
			cout<<"\nWasher code(a/b/c): ";
			cin>>code;
			cout<<"Washer temperature(C/W/H): ";
			cin>>temp;
			code=tolower(code);
			temp=toupper(temp);
		}
